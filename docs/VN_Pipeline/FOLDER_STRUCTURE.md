...
│
├── inference/                # Generation and testing scripts
│   ├── __init__.py
│   ├── seq2seq_infer.py          # Seq2SeqRewriter class - core inference
│   ├── rewrite_and_speak.py      # Full pipeline: paraphrase → voice
│   ├── test_seq2seq.py           # Test model with various settings
│   ├── generate_base.py          # CLI for base model generation
│   ├── generate_vn.py            # CLI for fine-tuned model generation
│   ├── analyze_training.py       # Analyze training metrics/graphs
│   ├── debug_weights.py          # Debug checkpoint loading
│   └── debug_weight_mapping.py   # Debug weight transfer from GPT
│
├── eval/                     # Evaluation
│   ├── __init__.py
...
| `test_seq2seq.py` | Test model with different temperature/top_k |

### Debug/Analysis

| File | Purpose |
|------|---------|
| `analyze_training.py` | Analyze metrics.csv, plot loss curves |
| `debug_weights.py` | Verify checkpoint loading works correctly |
| `debug_weight_mapping.py` | Check weight transfer from GPT to EncoderDecoder |
